...
assert.match(currentWorker, /aes-shell-v7/, 'production worker version is not v7');
await writeFile(workerPath, currentWorker.replaceAll('aes-shell-v7', 'aes-shell-update-old'));
...
try {
  ...
  assert.deepEqual(caches.filter((name) => name.startsWith('aes-shell-')), ['aes-shell-v7']);
  await browser.close();
  console.log('PASS update toast shown; v7 activated; old cache removed');
} finally {
  server.kill('SIGTERM');
