...
assert.match(currentWorker, /aes-shell-v6/, 'production worker version is not v6');
await writeFile(workerPath, currentWorker.replaceAll('aes-shell-v6', 'aes-shell-update-old'));
...
try {
  ...
  assert.deepEqual(caches.filter((name) => name.startsWith('aes-shell-')), ['aes-shell-v6']);
  await browser.close();
  console.log('PASS update toast shown; v6 activated; old cache removed');
} finally {
  server.kill('SIGTERM');
