...
assert.match(currentWorker, /aes-shell-v12/, 'production worker version is not v12');
await writeFile(workerPath, currentWorker.replaceAll('aes-shell-v12', 'aes-shell-update-old'));
...
try {
  ...
  assert.deepEqual(caches.filter((name) => name.startsWith('aes-shell-')), ['aes-shell-v12']);
  await browser.close();
  console.log('PASS update toast shown; v12 activated; old cache removed');
} finally {
  server.kill('SIGTERM');
